...
import { getQueue, setQueue as saveQueue, getNow, setNow as saveNow, isRemote } from '../lib/storage.js';
...
export default function Host() {
  ...
  // auto-advance via the YouTube embed's postMessage protocol
  useEffect(() => {
  ...
  return (
    <div id="app">
      ...
      <p className="tagline">Your speakers, the room's playlist. Scan to add — no account needed.</p>

      {!isRemote() && (
        <div className="file-warn">
          <b>Running in local-only mode.</b> No Firebase database is configured, so the queue only
          syncs on this device — guests scanning the QR code from their own phone won't see it. See
          the README for the two-minute setup that turns on real cross-device syncing.
        </div>
      )}
      ...
      <div className="host-grid">
        <div>
          <div className="stage-frame">
            ...
            <div className="now-info">
              ...
              <div className="now-title">{now ? now.title : 'Nothing yet — add a song to begin'}</div>
              <div className="now-sub">{now && now.author ? `from ${now.author}` : ''}</div>
            </div>
          </div>
          ...
          <div className="card" style={{ marginTop: 18 }}>
            <div className="setlist-head">
              <h2>Up next</h2>
              <span>
                {queue.length} {queue.length === 1 ? 'song queued' : 'songs queued'}
              </span>
            </div>
            <ul className="queue-list">
              {queue.length === 0 && (
                <div className="empty-note">The queue is empty. Share the QR code, or add one yourself below.</div>
              )}
              {queue.map((item, i) => (
                <li className="qitem" key={item.id}>
                  ...
                  <div className="qactions">
                    <button className="icon-btn" onClick={() => moveItem(i, -1)} title="Move up">↑</button>
                    <button className="icon-btn" onClick={() => moveItem(i, 1)} title="Move down">↓</button>
                    <button className="icon-btn" onClick={() => removeItem(i)} title="Remove">✕</button>
                  </div>
                </li>
              ))}
            </ul>
            ...
            <div className="host-controls">
              <button className="ghost-btn" onClick={playNext}>Skip song</button>
              <button className="ghost-btn" onClick={clearQueue}>Clear queue</button>
            </div>
        ...
        <div className="card join-card">
          ...
          <div className="qr-box">
            <img
              src={`https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${encodeURIComponent(guestUrl)}`}
